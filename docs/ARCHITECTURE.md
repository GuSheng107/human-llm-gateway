...
人工回复使用第三个统一表示 `normalized_reply`，包含 reasoning、tool calls 和 final text。Web 编辑器直接读写该结构；IM DSL 只负责把文本解析为同一结构；三个协议渲染器只从该结构生成 JSON/SSE。工具调用名称必须命中请求声明，参数必须是 JSON 对象并通过声明 Schema 校验；服务端按顺序生成 `call_01`、`call_02` 等 ID。提交成功后结构不可撤销或覆盖。
...
- 工作台向前端返回完整的调用方工具定义；选中工具时前端依据 JSON Schema 生成可编辑的最小参数骨架，不编造业务示例值。
