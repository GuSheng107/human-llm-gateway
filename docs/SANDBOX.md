...
> 文档状态：已部署版本安全边界（M12）
>
> M12 已完成并随当前版本部署。工具仍只在管理员白名单和用户显式确认下执行；调用方声明的工具不会进入本执行链路。
...
- 可选 stdin 传参：工具可声明一个 `stdin_parameter`，该参数的值经
  `docker run -i` 的 stdin 喂给容器（上限 64 KiB），不经 argv、不经 shell、
  其占位符不允许出现在命令模板中——用于 `tr`、`base64`、`sha256sum` 等
  从标准输入读取数据的文本处理与编码命令。
