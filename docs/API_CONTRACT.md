...
- NFC 归一化后哈希；允许空格和 Unicode 字符，但二者不参与三类复杂度判定，仅计入长度。
- 拒绝常见弱密码、与用户名相同或近似、明显部署默认词的 blocklist。
- 存储使用 Argon2id（`m=19456 KiB`、`t=2`、`p=1`）的 PHC 编码字符串；登录成功且参数低于当前策略时同流程重新哈希。
...
| 方法 | 路径 | 普通用户 | 管理员 |
| --- | --- | --- | --- |
| GET | `/api/im-platforms` | 查看平台、配置 Schema、绑定要求和固定绑定命令 | 查看 |
...
`POST /api/mcp/` 的 `tools/list` 与小助手使用相同工具注册表。新增：

| 工具 | 参数 | 输出 |
| --- | --- | --- |
| `get_request_view` | `task_id` 正整数 | 基于 RequestView 的当前输入、附带上下文、调用方 system、工具约束和风险确认状态；附件只给类型/数量，不发送原文、地址或请求 Header。 |
| `validate_reply_draft` | `task_id`、`draft`（ReplyDraft） | `valid`、安全错误原因；复用完整 Caller Tool 校验，覆盖空回复、调用 ID、名称、参数 Schema、tool_choice 和并行限制。成功同时返回风险确认状态，不代表提交成功。 |
| `get_trace_summary` | `trace_id`，1–64 字符 | 当前用户可见的最近 20 条日志/审计事件元数据、采样等级计数、`has_more`；不发送日志 message/context/detail 或审计正文。 |

任务内容必须由当前用户拥有，管理员通过这些工具也不能取得其他用户的请求正文。
trace 复用日志 Repository 的所有者范围。不存在和无权访问使用相同错误结果。
工具参数封闭校验；64 KiB、16 层深度及 10000 节点上限在 Schema 遍历之前执行。
输出递归过滤凭据键和已知密钥模式，HTTP URL 替换为 `[URL-OMITTED]`；文本和数组
限制长度，整体最大 32 KiB，发生截断时返回 `truncated=true`。不把有界摘要视为完整原文。
三协议工具循环、统一调用入口及不含参数正文的安全审计由小助手基础能力维护项提供；
工作台能力部署前先合入该维护项，不绕过其权限和审计入口。

`/replies` 上下文版本为 2，仅采集 `task_id`/`state_filter`；路由的 `focus`
与当前编辑器匹配时才携带 `unsaved_edit`。每次发送重新读取当前草稿，切换任务或
离开工作台不携带上一任务内容。不能解析的 JSON 草稿不以空对象伪装为有效草稿。
工具只读，不保存草稿、不确认风险、不提交回复，也不执行调用方工具。
