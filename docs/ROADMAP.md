...
- [ ] 邀请码撤销后立即不可用但继续显示；只有已撤销邀请码可软删除，注册来源和审计保留。
- [ ] 邀请码消费次数原子递增，并发注册不能突破使用上限。
- [ ] 用户使用邀请码注册和登录。
- [ ] 管理员直接创建、启用、禁用普通用户并重置密码。
- [ ] 禁用用户原子撤销会话和 API Key、终止活动任务、释放名额，并让外部等待者只收到通用错误。
- [ ] 用户修改自己的显示名和密码；`must_change_password=true` 登录后处于受限会话，改密成功后恢复完整权限。
- [ ] 普通用户只能访问自己的 IM、LLM 配置、API Key、任务和小助手会话。
- [ ] 管理员不能取得用户 Secret，也不能替用户回复。
- [ ] 后台不提供管理员提升接口；后续管理员通过受控 CLI 创建，最后一个有效管理员不能被禁用。
- [ ] 审计 action 使用稳定枚举，只展示操作者、动作、资源、时间、结果和字段名，不保存字段值或请求正文。
- [ ] 权限、邀请码并发、强制改密和受限会话测试通过。
...
- [ ] 配置应用和故障恢复只影响目标连接。
- [ ] 期望运行的长连接遇到普通网络错误时带抖动指数退避重连；`auth_required` 停止重试并等待所有者登录。
- [ ] 一个连接可被多个 API Key 选择。
...
- [ ] 用户从自己可见的 Fake Model 创建、编辑、删除和启停可复用模型分组，管理员可治理全部分组。
- [ ] 模型分组作为第一层候选集筛选，不能引用其他用户私有模型。
- [ ] API Key 可从分组预筛后的候选集中继续选择对外模型；未选择代表允许全部候选模型。
- [ ] `/v1/models` 必须鉴权，并返回按可见范围、模型分组和 Key 选择计算出的有效集合。
- [ ] 不存在、停用或不在有效集合的 Fake Model 返回对应协议的 `model_not_found`。
- [ ] `/v1/models` 与三个推理入口复用同一个 effective-model 查询和权限测试。

### M5-B：API Key、策略与用户级准入

- [ ] 用户创建、查看、禁用和删除自己的 API Key，明文只展示一次。
- [ ] API Key 独立配置 `human`、`llm`、`human_fallback_llm` 策略。
- [ ] API Key 选择 Web 或自己的一个 IM 连接；IM 能力未就绪时 Web 入口仍可独立工作。
- [ ] 人工超时支持 10 秒至 30 分钟，默认 300 秒。
- [ ] 停用或删除 Key 立即阻止新请求，已准入任务使用创建快照继续完成。
- [ ] 每个用户固定最多 10 个活动任务，所有 Key 和策略共用。
- [ ] 第 11 个并发请求原子拒绝并返回协议兼容的 429。
- [ ] 完成、失败、超时、取消和禁用用户正确且只释放一次名额。
- [ ] 多 Key、多策略并发测试证明无法绕过用户上限。
...
- [ ] 支持保存和恢复回复草稿。
- [ ] IM DSL 解析结果和 Web 编辑器加载同一个 ReplyDraft，结果往返不丢字段。
- [ ] 提交前提供预览和确认；首个有效提交后不可撤销、回退或覆盖。
- [ ] 人工提交完整回复后才开始伪流式输出。
