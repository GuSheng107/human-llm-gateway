...
### 🧰 工具调用透传
- 人工回复可引用调用方声明的工具（名称必须命中声明，否则 400）
- 网关只伪造并转发 tool call 输出：不执行、无白名单、无沙箱
- 工具由调用方自行声明并执行，后果由使用者自行承担
...
M12 原为隔离工具沙箱，现已移除。网关不再执行任何工具：人工回复只能引用调用方
