...
class LLMAdapter:
    ...
    def _headers(provider: LLMProvider, api_key: str) -> dict[str, str]:
        if provider.protocol == "anthropic":
            return {"x-api-key": api_key, "anthropic-version": "2023-06-01"} if api_key else {"anthropic-version": "2023-06-01"}
    ...
    async def list_models(self, provider: LLMProvider, app_secret: str) -> list[dict[str, Any]]:
        ...
        try:
            api_key = decrypt_secret(provider.api_key_encrypted, app_secret) if provider.api_key_encrypted else ""
        ...
        except (httpx.HTTPError, InvalidToken, KeyError, ValueError, TypeError,
                json.JSONDecodeError) as exc:
            raise LLMError("上游模型列表请求失败") from exc

    async def complete(self, provider: LLMProvider, model: str, messages: list[dict[str, Any]],
                       app_secret: str) -> list[ParsedEvent]:
        if provider.base_url.startswith("mock://"):
        ...
        try:
            ...
            request_options = {key: value for key, value in options.items()
            ...
            if provider.protocol == "anthropic":
                ...
                content = "".join(str(item.get("text", "")) for item in blocks
                                   if isinstance(item, dict) and item.get("type") == "text")
                reasoning = "".join(str(item.get("thinking", "")) for item in blocks
