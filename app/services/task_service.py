...
class TaskService:
    def __init__(
        self, settings: Settings, connector_manager: ConnectorManager | None = None
    ) -> None:
    ...
    def create_human_task(
        self, db: Session, key: ApiKey, protocol: str, model: str, request: dict[str, Any]
    ) -> RequestTask:
        route = key.route
        task = RequestTask(
            api_key_id=key.id,
        ...
        db.add(
            AuditLog(
                action="task.created",
                subject_type="request_task",
                subject_id=task.id,
                actor=key.prefix,
                detail_json=json.dumps({"mode": route.mode.value}),
            )
        )
        db.commit()
        ...
        if key.im_connection_id is None:
            return task
        target = self._connection_target(key)
        asyncio.create_task(
            self._deliver_human(key, OutboundTask(task_id=task.id, text=prompt, target=target))
        )
        return task
    ...
    async def _deliver_human(self, key: ApiKey, task: OutboundTask) -> None:
        try:
            assert key.im_connection is not None
    ...
    async def complete_llm_task(
        self, db: Session, task_id: str, messages: list[dict[str, Any]]
    ) -> list[ParsedEvent]:
        task = db.execute(
            select(RequestTask)
            .where(RequestTask.id == task_id)
            .options(
                joinedload(RequestTask.api_key)
        ...
        try:
            ...
            events = await self.llm.complete(
                provider, upstream_model, messages, self.settings.app_secret
            )
    ...
    def accept_reply(
        ...
        task = db.execute(
            select(RequestTask)
            .where(RequestTask.id == task_id)
            .options(joinedload(RequestTask.api_key))
        ).scalar_one_or_none()
        if task is None:
            raise TaskError("任务不存在")
        if external_message_id:
            duplicate = db.execute(
                select(RequestEvent).where(
                    ...
                    RequestEvent.external_message_id == external_message_id,
                )
            ).scalar_one_or_none()
            if duplicate:
                return [
                    ParsedEvent(e.kind, e.content, e.tool_name, e.tool_args_json)
...
def find_api_key(db: Session, secret: str) -> ApiKey | None:
    """用 prefix 索引预筛，消除 O(n) 全表遍历。"""
    if not secret or len(secret) < 8:
        return None
    prefix = secret[:12]
    candidates = (
        db.execute(select(ApiKey).where(ApiKey.active.is_(True), ApiKey.prefix == prefix))
        .scalars()
        .all()
    )
        ...
        if verify_api_key(secret, candidate.secret_hash):
            return db.execute(
                ...
                .options(
                    ...
                    joinedload(ApiKey.route).joinedload(ModelRoute.provider),
                )
            ).scalar_one()
    return None
