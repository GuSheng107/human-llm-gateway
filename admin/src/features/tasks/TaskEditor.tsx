...
export function TaskEditor({ taskId, onSubmitted, standalone = true }: TaskEditorProps) {
  ...
  useEffect(() => {
    listTools(1, true)
      .then((page) => {
        setTools(page.items);
        setSandboxAvailable(page.sandbox_available);
      })
      .catch(() => setTools([]));
  }, []);
  ...
  }, [task, reasoning, toolCalls, finalText, sandboxAvailable]);
  ...
  return (
    <div className={standalone ? "space-y-5" : "flex min-h-0 flex-1 flex-col gap-4"}>
        ...
        <section className="space-y-4">
          <Card>
            <div className="flex gap-1 border-b border-slate-100 px-4 pt-3">
              {TABS.filter((item) => sandboxAvailable || item.key !== "tools").map((item) => (
                <button
                  key={item.key}
                  type="button"
              ...
              {tab === "final" && (
                <div>
                  ...
                  <textarea
                    ...
                    disabled={!canEdit}
                    className="field-input min-h-[420px] text-sm"
                ...
                <div className="space-y-3">
                  {!sandboxAvailable && (
                    <p className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                      当前环境未检测到工具沙箱运行时（Docker/Podman），伪造工具调用已被禁用；
                      已有调用仅可移除，保存或提交时不能包含工具调用。
                    </p>
                  )}
                  {sandboxAvailable && (
          ...
          {!sandboxAvailable && (
            <Card>
              <p className="px-4 py-3 text-xs text-amber-700">
                未检测到工具沙箱运行时，伪造工具调用已被禁用；如需此能力请配置 Docker/Podman 与沙箱镜像。
              </p>
            </Card>
          )}
        </aside>
      ...
      {showGenerate && (
        <Modal
          title="调用 LLM 生成草稿"
          description="选择同协议 LLM 与生成模式；可逐条勾选参与生成的上下文消息。"
